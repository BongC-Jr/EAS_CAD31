   ...
   public class GenaiMethods
   {
      ...
      public static void AI_ConcBeamCapacity(List<object> loParam)
      {
         ...
         if (Convert.ToInt16(loParam[9]) < 0 || Convert.ToInt16(loParam[9]) > 5)
         {
            ...
            convo = $"\nmodel: Error - there is something wrong with the index value which is {Convert.ToInt16(loParam[9])}\n";
         ...
         if (Convert.ToInt16(loParam[11]) < 0 || Convert.ToInt16(loParam[11]) > 5)
         {
            ...
            convo = $"\nmodel: Error - there is something wrong with the index value which is {Convert.ToInt16(loParam[11])}\n";
            LogConversation(convo);
      ...
      public string ColumnBalanceCapacity(string ccParam)
      {
         /**
          *ccParam="B=400\nH=600\nNb=3\nNh=5\nfc=41.47\nfy=414\nDb=20\nDv=10"
          */
         string ccPyScript = ccParam + "\n" +
            ...
            "Ag=B*H\n" +
            "Nt=2*Nb+2*Nh-4\n" +
            "Ccb=0.85*fc*beta1*xb*B\n" +
            "Csb=Ash*(fy-0.85*fc)\n" +
            "Tb=Ash*fy\n" +
            "Pb=Ccb+Csb-Tb\n" +
            "ebb=(Ccb*(db-ab/2-dcb)+Cs*(db-dp-dcb)+T*dc)/Pb\n" +
            "Mbb=Pb*ebb\n" +
            "result=f'Pb={round(Pb/1000,2)}kN, Mbb={round(Mbb/1000000,2)}kN-m', ebb={round(ebb,3)}'";
         return ccPyScript;
      }
   }
}
